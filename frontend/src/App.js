    ...
    if (isAuthenticated && user) {
      ...
      socketRef.current.on('processing-complete', (data) => {
        ...
        if (data.result && !data.result.cancelled) {
          ...
          setProcessingDetails(data.result.processingDetails || null);
    ...
    <div className="container">
        ...
        <div className="card">
          ...
          >
            ...
            <div className="upload-hint">
              ...
              <small>Maximum file size: 2GB (audio is compressed before processing)</small><br/>
              <small style={{color: '#888'}}>💡 Large files are auto-compressed and split for fast transcription</small>
            </div>
          </div>
        </div>
      ...
      {(loading || activeJobs.size > 0) && (
        <div className="loading">
          {/* Show current upload progress only when no active jobs are being tracked */}
          {loading && activeJobs.size === 0 && (
          ...
          {processingDetails && (
            <div className="result-card">
              ...
              <div className="processing-details-grid">
                ...
                {processingDetails.originalFileSize && (
                  <div className="detail-item">
                    <span className="detail-label">Original size:</span>
                    <span className="detail-value">{processingDetails.originalFileSize}</span>
                  </div>
                )}
                {processingDetails.compressedFileSize && (
                  <div className="detail-item">
                    <span className="detail-label">Compressed size:</span>
                    <span className="detail-value">{processingDetails.compressedFileSize}</span>
                  </div>
                )}
                {processingDetails.compressionRatio && (
                  <div className="detail-item">
                    <span className="detail-label">Compression ratio:</span>
                    <span className="detail-value">{processingDetails.compressionRatio}</span>
                  </div>
                )}
                {processingDetails.chunksProcessed && (
                  <div className="detail-item">
                    <span className="detail-label">Chunks processed:</span>
                    <span className="detail-value">{processingDetails.chunksProcessed}</span>
                  </div>
                )}
                {processingDetails.totalProcessingTime && (
                  <div className="detail-item">
                    <span className="detail-label">Processing time:</span>
                    <span className="detail-value">{processingDetails.totalProcessingTime}</span>
                  </div>
                )}
                <div className="detail-item">
                  <span className="detail-label">Compression:</span>
                  <span className="detail-value">{processingDetails.conversionUsed ? 'Yes (mono MP3, 16kHz, 96kbps)' : 'No'}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Engine:</span>
                  <span className="detail-value">OpenAI GPT-4o Transcribe</span>
                </div>
              </div>
            </div>
          )}
